drop function if exists kaaj_request(date,date);
create function kaaj_request(fromdate date, todate date)
      ...
      from
          (select count(distinct da.id) as leave_count,office_code
           ...
           where lrd.is_active=true and lrd.status='A' and da.is_active=true
             and da.last_modified_date between fromDate and toDate
...
alter function kaaj_request(date, date) owner to postgres;
