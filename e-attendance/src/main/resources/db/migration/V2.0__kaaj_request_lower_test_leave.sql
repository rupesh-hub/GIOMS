drop function if exists kaaj_request_lower_test_leave(character varying,character varying,integer, integer,character varying, character varying);
create function kaaj_request_lower_test_leave(fromdate character varying, todate character varying ,limits integer, offsets integer,lowerOfficeList character varying, ordertype character varying)
    ...
    dynamicQuery :='select * from(select case when (lr.leave_count is null)then 0 else leave_count end as leave_count,
                  ...
                  from
                      ...
                      (select count(distinct da.id) as kaaj_count,office_code
                       ...
                       group by kr.office_code)kr  on kr.office_code=lr.office_code) q order by q.leave_count '||ordertype||' limit '''||limits||''' offset '''||offsets||''' ';
    return QUERY execute dynamicQuery;
END ;
$$;

alter function kaaj_request_lower_test_leave(character varying,character varying,integer, integer,character varying, character varying) owner to postgres;