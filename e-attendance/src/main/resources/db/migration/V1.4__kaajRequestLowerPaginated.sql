drop function if exists kaaj_request_lower_paginated(character  varying, character varying,integer,integer,character varying);
create function kaaj_request_lower_paginated(fromdate character varying, todate character varying, offsets integer, limits integer, lowerofficelist character varying)
    ...
    dynamicQuery :='(select case when (lr.leave_count is null)then 0 else leave_count end as leave_count,
                  ...
                  from
                      (select count(distinct da.id) as leave_count,office_code
                       ...
                       group by lr.office_code order by leave_count desc limit '''||limits||'''  offset '''||offsets||''')as lr
                          left join
...
alter function kaaj_request_lower_paginated(varchar, varchar, integer, integer, varchar) owner to postgres;
