package com.gerp.usermgmt.config.interceptor.auth;
...
public class RequestAuthorizationInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(
        ...
        if (tokenProcessorService.isAdmin() || isValidHeaders(request) || authorizeSubClients(request)) {
            return true;
        }
//        if (Boolean.FALSE.equals(moduleApiMappingMapper.isUserAuthorized((String) request.getAttribute(HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE),
...
//        }
    ...
    // verify if it stays in memory or removes
    ...
    //todo refactor with proper bean and scable for other client
    private boolean authorizeSubClients(HttpServletRequest request) {
        String clientId = tokenProcessorService.getClientId();
        if(clientId != null && clientId.equals("TMS-APP") || clientId.equals("QR-APP")) {
            return tmsClientValidator(request);
        } else {
            return false;
        }
    }

    // refactor with proper gerneric bean adapter and db use
    private boolean tmsClientValidator(HttpServletRequest request) {
      String uri = (String) request.getAttribute(HandlerMapping.BEST_MATCHING_PATTERN_ATTRIBUTE);
       if(Pattern.matches("^/tms/api/.*", uri) || Pattern.matches("^/dispatch-view-log/.*", uri)){
           return true;
       } else {
           throw new UnAuthenticateException(customMessageSource.get("client.unauthorized.access"));
       }
    }
    // todo create generic client validator for different client
