...
in (select dispatch_id  from memo_reference mr where referenced_memo_id  = v_id)) and rlf2.receiver_section_id = v_code
union
select distinct receiver_pis_code from draft_share ds
where ds.memo_id in(select mr.memo_id  from memo_reference mr where referenced_memo_id  = v_id) and ds.receiver_section_code = v_code and ds.is_active = true
union
select distinct receiver_pis_code from draft_share ds
where ds.dispatch_id in(select mr.dispatch_id from memo_reference mr where referenced_memo_id = v_id) and ds.receiver_section_code = v_code and ds.is_active = true
) tt
