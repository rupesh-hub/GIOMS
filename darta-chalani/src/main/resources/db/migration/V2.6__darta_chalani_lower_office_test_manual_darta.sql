CREATE OR REPLACE FUNCTION public.darta_chalani_lower_office_test_manual_darta(fromdate character varying, todate character varying, limits integer, offsets integer, lowerofficelist character varying, ordertype character varying)
        ...
        (select q1.office_code as officeCode,
         ...
         from
            ...
            select unnest('''||lowerofficelist||''' :: varchar[]) as office_code) tt on t.office_code = tt.office_code
             ...
             )as ti on ti.m_office_code=dr.office_code ) q1 order by q1.manual '||ordertype||' limit '''||limits||''' offset '''||offsets||''')
