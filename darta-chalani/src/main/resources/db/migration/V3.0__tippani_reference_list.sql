CREATE OR REPLACE FUNCTION public.tippani_reference_list(v_id bigint)
...
BEGIN
    dynamicQuery :='select tt.referenced_memo_id from
       ...
       WITH RECURSIVE a AS (
        ...
        WHERE mr.memo_id  = '''||v_id||''' and mr.is_active = true and mr.referenced_memo_id is not null
        UNION ALL
        SELECT d.referenced_memo_id,d.memo_id
        FROM memo_reference d
            INNER JOIN a ON a.referenced_memo_id  = d.memo_id where d.referenced_memo_id is not null
        )select * from a
        ) t
        UNION ALL
        select * from(
        ...
        FROM memo_reference mr
        WHERE mr.referenced_memo_id = '''||v_id||''' and mr.is_active = true and mr.memo_id is not null
        UNION ALL
        SELECT d.memo_id  as referenced_memo_id,d.memo_id
        FROM memo_reference d
            INNER JOIN a ON a.memo_id = d.referenced_memo_id where d.memo_id is not null
        )select * from a)
        t
        union all
