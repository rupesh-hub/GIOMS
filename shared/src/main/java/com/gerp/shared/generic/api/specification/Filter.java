package com.gerp.shared.generic.api.specification;
...
public class Filter {
    ...
    private static Filter parse(String[] params, int s, int e) {
        int n = e - s + 1;
        if (n % 4 != 3)
            throw new com.gerp.shared.generic.api.specification.SpecificationException("illegal" + String.join(DELIMITER, Arrays.asList(params).subList(s, e + 1)));
            ...
            if (params[i].startsWith(LEFT_BRACKET)) {
                ...
                if (i + 3 <= e)
                    if (!params[i + 3].equals(logic) || !params[i + 3].equals(AND) && !params[i + 3].equals(OR))
                        throw new com.gerp.shared.generic.api.specification.SpecificationException("Illegal logic or mixed logic in one level bracket");
    ...
    private static int findRightBracket(String[] params, int i, int e) {
        ...
        throw new com.gerp.shared.generic.api.specification.SpecificationException("");//TODO
    }
}
